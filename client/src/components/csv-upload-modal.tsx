...
const createBulkQuestionsInFlask = async (chapterId: number, questions: any[]) => {
  const response = await fetch(`${FLASK_API_URL}/api/questions/bulk`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ chapterId, questions })
  });
  if (!response.ok) {
...
import { createBulkQuestions } from "@/lib/db-persistent-functions";
...
export function CSVUploadModal({ isOpen, onClose, chapterId, chapterTitle }: CSVUploadModalProps) {
  ...
  const handleUploadQuestions = async (questions: ParsedQuestion[]) => {
    ...
    try {
      ...
      const result = await createBulkQuestionsInFlask(chapterId, formattedQuestions);

      console.log('Upload result:', result);

      toast({
        title: "Success!",
        description: `Successfully uploaded ${formattedQuestions.length} questions to ${chapterTitle}`,
      });
