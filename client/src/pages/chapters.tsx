...
import { getSubjects } from "@/lib/db-persistent-functions";
import { DeleteConfirmationModal } from "@/components/delete-confirmation-modal";

// Flask API URL
const FLASK_API_URL = 'https://d33b02ca-b16d-48ca-97df-a8dd5f622b71-00-j3w4z2qff2ue.sisko.replit.dev';

// Flask API functions
const getChaptersFromFlask = async () => {
  const response = await fetch(`${FLASK_API_URL}/api/chapters`);
  if (!response.ok) throw new Error('Failed to fetch chapters');
  return response.json();
};

const createChapterInFlask = async (data: any) => {
  const response = await fetch(`${FLASK_API_URL}/api/chapters`, {
    method: 'POST',
...
const deleteChapterFromFlask = async (id: number) => {
  const response = await fetch(`${FLASK_API_URL}/api/chapters/${id}`, {
    method: 'DELETE'
  });
...
const getQuestionsByChapterFromFlask = async (chapterId: number) => {
  const response = await fetch(`${FLASK_API_URL}/api/questions/chapter/${chapterId}`);
...
export default function Chapters() {
  ...
  const { data: questions = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const chapter of chapters) {
        try {
          const chapterQuestions = await getQuestionsByChapterFromFlask(chapter.id);
  ...
  const createChapterMutation = useMutation({
    mutationFn: async (data: z.infer<typeof formSchema>) => {
      ...
      return await createChapterInFlask(chapterData);
    },
    onSuccess: () => {
      toast({
        title: "Success",
  ...
  const deleteChapterMutation = useMutation({
    mutationFn: async (chapterId: number) => {
      return await deleteChapterFromFlask(chapterId);
    },
    onSuccess: () => {
      refetch();
      toast({
